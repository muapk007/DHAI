...
class MaintenanceEquipmentInherit(models.Model):
    ...
    def action_transfer_independent(self, record):
        ...
        wh = self.env['stock.location'].search([('name', '=', 'Stock')])
        site_loc = self.env['stock.location'].search([('name', '=', record.location_id.name)])
        ...
        for cctv_line in record.cctv_installed:
            line = self.env['stock.move.line'].create({
                'product_id': cctv_line.id,
                'product_uom_qty': 1,
                'product_uom_id': cctv_line.uom_id.id,
                'location_id': wh.id,
                'location_dest_id': site_loc.id,
                'qty_done': 1,
                'picking_id': rec.id,
            })
        for no_of_ptz_line in record.no_of_ptz:
            line2 = self.env['stock.move.line'].create({
                'product_id': no_of_ptz_line.id,
                'product_uom_qty': 1,
                'product_uom_id': no_of_ptz_line.uom_id.id,
                'location_id': wh.id,
                'location_dest_id': site_loc.id,
                'qty_done': 1,
                'picking_id': rec.id,
            })
        for no_of_fibre_line in record.no_of_fibre:
            line3 = self.env['stock.move.line'].create({
                'product_id': no_of_fibre_line.id,
                'product_uom_qty': 1,
                'product_uom_id': no_of_fibre_line.uom_id.id,
                'location_id': wh.id,
                'location_dest_id': site_loc.id,
                'qty_done': 1,
                'picking_id': rec.id,
